...
# مجلدات الوجهة
AGENTS_DEST = APEX_HOME / "agents"
SKILLS_DEST = APEX_HOME / "skills"
RULES_DEST = APEX_HOME / "rules"
HOOKS_DEST = APEX_HOME / "hooks"
EXAMPLES_DEST = APEX_HOME / "examples"
TEMPLATES_DEST = APEX_HOME / "templates"
SHIELD_DEST = APEX_HOME / "shield"
INSTINCTS_DEST = APEX_HOME / "instincts"
MEMORY_DEST = APEX_HOME / "memory"
...
def init_database():
    ...
    conn.commit()
    conn.close()
    print("✅ تم تهيئة قاعدة البيانات Apex")
...
def validate_paths():
    ...
    print(f"✅ مجلدات Apex جاهزة: {APEX_HOME}")
    return True
...
def generate_summary_report(agent_count: int, skill_count: int, rule_count: int, 
    ...
    """إنشاء تقرير ملخص"""
    report_path = APEX_HOME / "import_report.json"
    report = {
        "timestamp": datetime.now().isoformat(),
        "source": str(SOURCE),
        "destination": str(APEX_HOME),
        "counts": {
            "agents": agent_count,
            "skills": skill_count,
            "rules": rule_count,
    ...
    print("\n" + "="*60)
    print("🏔️ Apex - تقرير الاستيراد")
    print("="*60)
    print(f"🕒 الوقت: {datetime.now()}")
    print(f"📁 المصدر: {SOURCE}")
    print(f"📁 الوجهة: {APEX_HOME}")
...
def main():
    print("="*60)
    print("🏔️ Apex Resource Copier v1.0")
    ...
    print("\n🎉 تم استيراد جميع الموارد إلى Apex بنجاح!")
    print(f"🏔️ Apex جاهز للتشغيل!")
    print(f"\n💡 التشغيل: python apex.py")
